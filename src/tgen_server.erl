...
-optional_callbacks([format_status/2, before_commit/4, before_rollback/5, after_commit/4, after_rollback/4]).

%% @doc Start tgen_server process.
%%
%% There are start and start_link functions similar to the gen_server
%% with additional parameter Tr - transaction::transactwidthion()
%%
%% @end
...
handle_call({Tr, commit_1}, From, #{module := Mod, tr_state := active, active_tid := ATID} = State) ->
  ActiveVState = maps:get(ATID, State),
  R =
    case erlang:function_exported(Mod, before_commit, 4) of
      true ->
        Mod:before_commit(Tr, From, active, ActiveVState);
      _ ->
        true
    end,
  NewState =
...
handle_call({_Tr, commit_1}, _From, State) ->
  {reply, true, State};
...
handle_call(
  ...
  case erlang:function_exported(Mod, after_commit, 4) of
    true ->
      Mod:after_commit(Tr, From, TRState, CommittedVState);
    _ ->
      continue
  end,
...
handle_call(
  ...
  ActiveVState = maps:get(ATID, State),
  case erlang:function_exported(Mod, before_rollback, 5) of
    true ->
  ...
  CommittedVState = maps:get(CTID, State),
  case erlang:function_exported(Mod, after_rollback, 4) of
    true ->
      Mod:after_rollback(Tr, From, TRState, CommittedVState);
    false ->
      continue
  end,
  demonitor_client(MonitorRef),
...
handle_call(
  {#{tr_id := TrID, tr_bet := Bet} = Tr, lock},
  {From, _Ref},
  #{tr_state := committed, module := _Mod, committed_tid := CTID} = State
) ->
  CommittedVState = maps:get(CTID, State),
  ...
  NewState = State#{
    tr_state => active,
    active_bet => Bet,
    active_client_pid => From,
    active_tid => TrID,
    TrID => CommittedVState,
    monitor_ref => ClientRef
  },
  transaction:write_transaction_log(Tr, self()),
  {reply, ok, NewState};
handle_call(
  {#{tr_id := TrID} = _Tr, lock},
  _From,
  #{tr_state := TrSt, active_tid := ATID} = State
) when
  ...
  {reply, ok, State};
handle_call(
  {#{tr_id := TrID, i_level := no_record_version, wait := wait} = _Tr, lock},
  {From, _Tag},
  #{tr_state := active, active_tid := ATID} = State
) when
  TrID =/= ATID
  ->
  ...
  {reply, locked, State};
handle_call(
  {#{tr_id := TrID, i_level := IL, wait := W} = _Tr, lock},
  _From,
  #{tr_state := active, active_tid := ATID} = State
) when
...
handle_call(
  ...
  {reply, ok, NewState};
handle_call(
  {#{tr_id := TrID, i_level := record_version, wait := _W} = _Tr, stop},
  _From,
  #{tr_state := active, active_tid := ATID} = State
) when
  TrID =/= ATID
  ...
  {reply, busy, State};
handle_call(
  {#{tr_id := TrID, i_level := no_record_version, wait := no_wait} = _Tr, stop},
  _From,
  #{tr_state := active, active_tid := ATID} = State
) when
  TrID =/= ATID
  ...
  {reply, busy, State};
handle_call(
  {#{tr_id := TrID, i_level := no_record_version, wait := wait} = _Tr, stop},
  {From, _Tag},
  #{tr_state := active, active_tid := ATID} = State
) when
  TrID =/= ATID
  ...
  {reply, locked, State};
handle_call(
  {#{tr_id := TrID, tr_bet := Bet, overwrite := OWrite} = Tr, Request},
  {From, Ref},
  #{tr_state := committed, module := Mod, committed_tid := CTID} = State
) ->
  CommittedVState = maps:get(CTID, State),
...
handle_call(
  {#{tr_id := TrID} = _Tr, _Request},
  _From,
  #{tr_state := TrSt, commit_phase := 1, active_tid := ATID} = State
) when
  TrID =/= ATID, TrSt =:= starting;
  TrID =/= ATID, TrSt =:= active;
  TrID =/= ATID, TrSt =:= stopping
...
handle_call(
  ...
  {reply, busy, State};
handle_call(
  {#{tr_id := TrID, i_level := record_version, wait := _W, overwrite := OWrite} = Tr, Request},
  From,
  #{tr_state := TrSt, committed_tid := CTID, active_tid := ATID, module := Mod} = State
) when
  TrID =/= ATID, TrSt =:= active;
  TrID =/= ATID, TrSt =:= stopping
  ->
...
handle_call(
  {#{tr_id := TrID, i_level := no_record_version, wait := no_wait} = _Tr, _Request},
  _From,
  #{tr_state := active, active_tid := ATID} = State
) when
  TrID =/= ATID
  ...
  {reply, busy, State};
handle_call(
  {#{tr_id := TrID, i_level := no_record_version, wait := wait} = _Tr, _Request},
  {From, _Tag},
  #{tr_state := active, active_tid := ATID} = State
) when
  TrID =/= ATID
